...
public class SumPair_of_ArrayElement {
	...
	public void doquicksort(int [] a, int lower, int upper){   //overall best(except sorted Array and small number of item in Array)
	   ...
	   public int partition(int[] a , int lower, int upper, int pivot){
		  ...
		  /* int lowerptr=lower;
		   int upperptr=upper-1;
		   int pivot2 =a[upper];
		   
		   while(lowerptr < upperptr){
			   
			   while(lowerptr<upper && a[lowerptr] < pivot2){   // here we know pivot at last pointer may cross boundary , so need to check
				   lowerptr++;
			   }
			   
			   while(upperptr>lower && a[upperptr] > pivot2){
				   upperptr--;
			   }
